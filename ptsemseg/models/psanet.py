...
#from ptsemseg.models.utils_for_psanet.psa import functional as PF
from ptsemseg.models.utils_for_psanet import resnet as models  


class PSA(nn.Module):
    def __init__(self, in_channels=2048, mid_channels=512, psa_type=2, compact=True, shrink_factor=2, mask_h=59,
...
class PSANet(nn.Module):
    def __init__(self, layers=50, dropout=0.1, classes=19, zoom_factor=8, use_psa=True, psa_type=2, compact=True,
                 shrink_factor=2, image_size=(1025,2059), normalization_factor=1.0, psa_softmax=True, pretrained=True):
        ...
        self.mask_h = (images_size[0] - 1)// (8 *shrink_factor) + 1
        self.mask_w = (images_size[1] - 1)// (8 *shrink_factor) + 1
        ...
        fea_dim = 2048
        if use_psa:
            self.psa = PSA(fea_dim, 512, psa_type, compact, shrink_factor, self.mask_h, self.mask_w, normalization_factor, psa_softmax)
            fea_dim *= 2
    ...
    def forward(self, x, y=None):
        ...
        if(original_size[0] != self.image_size[0] or original_size[1] !=self.image_size[1]):
            print("input size not expected by psanet. F.interpolate used") #TODO REMOVE
            x = F.interpolate(x, size=(self.image_size[0], self.image_size[1]), mode='bilinear', align_corners=True)
