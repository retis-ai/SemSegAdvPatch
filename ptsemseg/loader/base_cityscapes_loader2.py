...
#         print(image_ratio)
        ...
        to_remove_width_per_side =  int(image_ratio/2 * (bias + bot))  - 300 #int((self.bottom_crop*image_ratio)/2)
#         print(to_remove_width_per_side)
        
#         print("DIMENSIONS: %.3f, %.3f, %.3f, %.3f" % (bias, to_remove_width_per_side, self.img_size[0] - bot - bias, self.img_size[1] - 2 * to_remove_width_per_side))

        
#         img = crop(
#             img, 
#             top = self.bottom_crop + bias, 
#             left = to_remove_width_per_side, 
#             height = self.img_size[0] - 2*self.bottom_crop - bias,
...
#         )
        ...
        img = crop(
            img, 
            top=bias, left=to_remove_width_per_side, height=self.img_size[0] - bot - bias, width=self.img_size[1] - 2 * to_remove_width_per_side
        )
...
#         img = F.interpolate(img.unsqueeze(0), size=self.img_size, mode="bilinear", align_corners=True).squeeze(0)
    ...
    def image_transform(self, img, resizecrop=True):
        if resize is True:
...
if __name__ == "__main__":
    ...
    augmentations = Compose([Scale(2048),crop RandomRotate(10), RandomHorizontallyFlip(0.5)])
